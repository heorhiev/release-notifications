...
final class GeminiPromptBuilder
{
    ...
    {
        $this->maxIssues = (int) (Env::get('GEMINI_SUMMARY_MAX_ISSUES', '100') ?? '100');
        $this->maxInputChars = (int) (Env::get('GEMINI_SUMMARY_MAX_INPUT_CHARS', '60000') ?? '60000');
        $this->maxSummaryChars = (int) (Env::get('GEMINI_SUMMARY_MAX_SUMMARY_CHARS', '400') ?? '400');
        $this->maxDescriptionChars = (int) (Env::get('GEMINI_SUMMARY_MAX_DESCRIPTION_CHARS', '700') ?? '700');
    }
    ...
    public function instructions(): string
    {
        ...
        if (!is_string($prompt) || trim($prompt) === '') {
            throw new \RuntimeException('Invalid Gemini system prompt template');
        }
    ...
    public function build(string $release, array $issues): array
    {
        ...
        foreach ($issues as $index => $issue) {
            ...
            if (!is_string($promptCandidate) || trim($promptCandidate) === '') {
                throw new \RuntimeException('Invalid Gemini input prompt template');
            }
